 ...
 */
export async function generateWithRetry(params: any, retries = 4, delay = 2000): Promise<GenerateContentResponse> {
  let lastError;
  for (let i = 0; i < retries; i++) {
    try {
      ...
      const isServerOverload = error.status === 503 || error.code === 503 || errorMsg.includes("503") || errorMsg.includes("overloaded");

      if (i === retries - 1) break;

      if (isRateLimit || isServerOverload) {
         console.warn(`API call failed (attempt ${i + 1}/${retries}) due to overload. Retrying in ${delay * Math.pow(2, i)}ms...`, error.message);
